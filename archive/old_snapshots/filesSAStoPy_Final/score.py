...
def run_scoring(df: pd.DataFrame, coeff_dir: str | Path, config) -> pd.DataFrame:
    ...
    Returns
    -------
    DataFrame with BTSMOdds, ProbToWin, ValBetIf, rank, etc.
    """
...
def _score_maiden(df: pd.DataFrame, coeff_dir: Path, config) -> pd.DataFrame:
    ...
    result["predicted"] = (
        result["score1"].fillna(0) * 0.50 +
        result["score2"].fillna(0) * 0.25 +
        result["score3"].fillna(0) * 0.25
    )
    result["predicted"] = result["predicted"].replace(0, np.nan)
...
def _normalize_probabilities(df: pd.DataFrame) -> pd.DataFrame:
    """
    Handle coupled entries, normalize win probabilities, compute BTSMOdds.
...
def _build_output(df: pd.DataFrame) -> pd.DataFrame:
    """
    Compute final output columns: BTSMOdds, ProbToWin, ValBetIf, rank, etc.
    Mirrors: data roi2 in SAS lines 2960–3109.
    """
    df = df.copy()

    # BTSMOdds — rounded to nearest reasonable increment
    df["BTSMOdds"] = np.where(
    ...
    # Round po_s to same increments as BTSMOdds
    df["po_s"] = np.where(
        df["po_s"] < 1.9,
...
OUTPUT_COLUMNS = [
    ...
    "Race", "Num", "Horse", "rank", "BTSMOdds", "MornOdds",
