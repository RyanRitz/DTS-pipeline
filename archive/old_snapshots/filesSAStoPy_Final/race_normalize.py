"""
BTSM Pipeline — race_normalize.py
====================================
...
def _compute_prerequisites(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(1, 11):
        fp  = df.get(f"FinishPosition{i}", pd.Series("", index=df.index)).fillna("")
        ...
        mp  = df.get(f"MoneyPosition{i}", pd.Series("", index=df.index)).fillna("")
...
def _compute_race_averages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    var_file = Path(__file__).parent.parent / "pipeline" / "race_norm_vars.txt"
    ...
    if var_file.exists():
        ...
        logger.warning("race_norm_vars.txt not found — using subset")
        all_vars = []
